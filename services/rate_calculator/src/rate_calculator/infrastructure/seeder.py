...
async def seed_district_risk_configs(session: AsyncSession, csv_path: str) -> None:
    """Seeds all districts with a neutral multiplier of 1.0.

    Replace this function with an actual risk-scoring strategy once defined.
    """
    ...
    print(f"Seeded {len(district_list)} district risk configs (multiplier=1.0)")
...
async def _run(csv_path: str) -> None:
    ...
    async with session_factory() as session:
        ...
        await seed_district_risk_configs(session, csv_path)
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: seed <path/to/districts.csv>")
        sys.exit(1)
